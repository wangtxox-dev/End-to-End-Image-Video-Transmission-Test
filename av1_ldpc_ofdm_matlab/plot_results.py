"""
plot_results.py  —  JSCC 悬崖效应极速可视化看板 [OFDM + TDL-C 版]
从最新的 sim_av1_ofdm_data_*.pkl 存档中读取仿真数据并生成图表。
...
if not candidates:
    raise FileNotFoundError(
        "未找到任何仿真存档文件 (sim_av1_ofdm_data_*.pkl)。\n"
...
ax.set_title('MS-SSIM vs SNR (AV1/AVIF + 5G NR LDPC + OFDM TDL-C)', fontsize=13)
ax.legend(fontsize=9, loc='upper left')
ax.grid(True, alpha=0.35)
ax.set_ylim(-0.05, 1.05)
ax.set_xlim(global_min_snr - 0.5, global_max_snr + 0.5)

# 根据开关状态和时间戳动态生成文件名（ofdm 前缀防止覆盖 AWGN 结果）
prefix = 'cliff_effect_av1_ofdm'
