"""
任务1: 验证 JPEG 压缩 R-D 曲线是否与理论公式吻合
理论公式: MS-SSIM = 1 - 0.02155 * bpp^(-1.46466)
"""
import io
import os
import glob
...
# 参数配置
# ─────────────────────────────────────────────
KODAK_DIR = os.path.join(os.path.dirname(__file__), 'kodak')
G          = 768 * 512 * 2         # 总空口比特数，Kodak 全分辨率
rates      = [1/8, 1/4, 1/3, 1/2]  # 目标码率列表


def jpeg_bisect(img_pil: Image.Image, target_bits: int) -> bytes:
    """二分查找严格不超过 target_bits 的最高 JPEG quality，返回二进制流"""
    lo, hi = 1, 95
    best_buf = None
    while lo <= hi:
        mid = (lo + hi) // 2
...
def theory_msssim(bpp: float) -> float:
    return 1.0 - 0.02155 * (bpp ** (-1.46466))
...
for R in rates:
    K_target = int(G * R)
    bpp_target = K_target / (768 * 512)  # = R * 3，基于 Kodak 全分辨率像素数
    ...
    for fpath in png_files:
        ...
        # 二分控码（严格 <= target_bits）
        jpeg_bytes = jpeg_bisect(img, K_target)
        actual_bits = len(jpeg_bytes) * 8
        actual_bpp  = actual_bits / (768 * 512)

        # 解压回图片
        img_rec = Image.open(io.BytesIO(jpeg_bytes)).convert('RGB')
...
# 自我验证判断
# ─────────────────────────────────────────────
max_err = max(abs(np.array(measured_msssim) - np.array([theory_msssim(b) for b in measured_bpps])))
print(f"\n最大绝对误差: {max_err:.6f}")
if max_err < 0.05:
    print("✅ 验证通过：散点与理论曲线高度吻合（最大误差 < 0.05），自动继续执行任务2和3。")
    VERIFIED = True
else:
    print("⚠️  验证警告：误差偏大，请检查 JPEG 控码逻辑。")
    VERIFIED = False
